package com.antik;
  ...
  public class Main {
      public static void main(String[] args) {
          ...
          try {
              ...
              if (inputPath.endsWith(".apk")) {
                  ...
                  // Print package name so callers (e.g. PairipX) can parse it
                  try {
                  ...
                  mergedApkFile = resolveOut(inputApk, name, outDir);
              ...
              if (translatePath != null) {
                  ...
                  if (jsonFile.exists()) {
                      ...
                      tn = (td > 0 ? tn.substring(0, td) : tn) + "_patched.apk";
                      File transFile = resolveOut(inputApk, tn, outDir);

                      System.out.println("[BUILD] Building Patched APK...");
                      writeSilently(module, transFile);
                      System.out.println("[BUILD] Patched APK built at: " + transFile.getAbsolutePath());
                  } else {
                  ...
                  File paiFile = resolveOut(inputApk, pn, outDir);
      ...
      /** Resolve output file: use outDir if set, otherwise place next to inputApk. */
      private static File resolveOut(File inputApk, String name, File outDir) {
          if (outDir != null) return new File(outDir, name);
          return new File(output.get_out(inputApk, name));
      }

      private static void writeWithProgress(ApkModule module, File file) throws IOException {
          int total = module.getZipEntryMap().size();
          if (module.hasTableBlock()) total--;
          int[] count = {0};
